...
public class SphereLight implements LightSource, Shader {
    ...
    public boolean update(ParameterList pl, SunflowAPI api) {
        ...
        typ = pl.getString("balltype", typ);
        ballcolor = pl.getColor("ballcolor", ballcolor);
        numSamples = pl.getInt("samples", numSamples);
        radius = pl.getFloat("radius", radius);
        r2 = radius * radius;
        center = pl.getPoint("center", center);
        noball = pl.getBoolean("noball", noball);
        return true;
    }
    ...
    public void init(String name, SunflowAPI api) {
        ...
        if (noball) {
            api.shader(name + ".shader", new TransparentShader());
        } else {
            switch (typ) {
                case "diffuse":
                    api.parameter("diffuse", ballcolor);
                    api.shader(name + ".shader", new DiffuseShader());
                    break;
                case "mirror":
                    api.parameter("color", ballcolor);
                    api.shader(name + ".shader", new MirrorShader());
                    break;
                case "plastic":
                    api.parameter("color", ballcolor);
                    api.shader(name + ".shader", new PlasticShader());
                    break;
                case "neon":
                    api.parameter("color", ballcolor);
                    api.shader(name + ".shader", new NeonShader());
                    break;
                case "stainedmirror":
                    api.parameter("color", ballcolor);
                    api.shader(name + ".shader", new StainedMirrorShader());
                    break;
                case "metallic":
                    api.parameter("color", ballcolor);
                    api.shader(name + ".shader", new MetallicShader());
                    break;
                case "ambient":
                    api.parameter("bright", ballcolor);
                    api.parameter("dark", Color.BLACK);
                    api.parameter("samples", 16);
                    api.parameter("maxdist", 100f);
                    api.shader(name + ".shader", new AmbientOcclusionShader());
                    break;
                case "ambient_rev":
                    api.parameter("bright", Color.BLACK);
                    api.parameter("dark", ballcolor);
                    api.parameter("samples", 16);
                    api.parameter("maxdist", 100f);
                    api.shader(name + ".shader", new AmbientOcclusionShader());
                    break;
                case "shiny":
                    api.parameter("diffuse", ballcolor);
                    api.parameter("reflection", 0.5f);
                    api.shader(name + ".shader", new ShinyDiffuseShader());
                    break;
                case "glass":
                    api.parameter("color", ballcolor);
                    api.parameter("eta", 1.6f);
                    api.shader(name + ".shader", new GlassShader());
                    break;
                case "constant":
                default:
                    api.parameter("color", ballcolor);
                    api.shader(name + ".shader", new ConstantShader());
                    break;
            }
        }
