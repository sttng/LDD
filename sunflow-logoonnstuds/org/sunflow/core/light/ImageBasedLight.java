...
public class ImageBasedLight implements PrimitiveList, LightSource, Shader {
    ...
    public Color getRadiance(ShadingState state) {
        ...
        if (power >= 1) {
            farba = getColor(basis.untransform(state.getRay().getDirection(), new Vector3())).mul((float) power);
        } else if (power < 0) {
            farba = getColor(basis.untransform(state.getRay().getDirection(), new Vector3())).mul((float) power);
        } else {
            farba = getColor(basis.untransform(state.getRay().getDirection(), new Vector3()));
        }
        return state.includeLights() ? farba : Color.BLACK;
    }

    private Color getColor(Vector3 dir) {
        float u, v;
        // assume lon/lat format
        double phi = Math.acos(dir.y);
