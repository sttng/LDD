...
class LightServer {
    ...
    public static float transShadowsDepth;
    ...
    boolean build(Options options) {
        ...
        transShadowsDepth = options.getFloat("depths.transshadow", transShadowsDepth);
        ...
        transShadowsDepth = MathUtils.clamp(transShadowsDepth, 0f, 1f);
        ...
        if (giEngine != null && !giEngine.init(scene))
                return false;
    ...
    Color shadeHit(ShadingState state, Color defaultz) {
        state.getInstance().prepareShadingState(state);
        Shader shader = getShader(state);
        return (shader != null) ? shader.getRadiance(state) : defaultz; // new Color(0.3f, 0.6f, 0.976f);
    }
    ...
    Color traceGlossy(ShadingState previous, Ray r, int i) {
        ...
        IntersectionState istate = previous.getIntersectionState();
        scene.trace(r, istate);
        return istate.hit() ? shadeHit(ShadingState.createGlossyBounceState(previous, r, i), Color.BLACK) : Color.BLACK;
    }
    ...
    /* ------------------------- G L A S S   S T U D S   D A R K N E S S -------------------------- */
    Color traceRefraction(ShadingState previous, Ray r, int i) {
        ...
        if (previous.getRefractionDepth() >= maxRefractionDepth || previous.getDiffuseDepth() > 0){
            try {
                Shader s = previous.instance.getShader(0);
                if(s instanceof GlassShader)
                    /* ---------- STUDS ---------- */
                    return Color.WHITE;
                else
                    return Color.BLACK;
        ...
        try {
            ...
            if(istate.hit()){
                if(s instanceof GlassShader)
                    /* ------------------------------ MAIN GLASS DARKNESS ------------------------------------ */
                    return shadeHit(ShadingState.createRefractionBounceState(previous, r, i), Color.WHITE);
                else
                    return shadeHit(ShadingState.createRefractionBounceState(previous, r, i), Color.BLACK);
            }else{
                return Color.BLACK;
            }
        } catch (UnsupportedOperationException e) {
            return istate.hit() ? shadeHit(ShadingState.createRefractionBounceState(previous, r, i), Color.BLACK) : Color.BLACK;
        } catch (Exception e) {
            //e.printStackTrace();
            return istate.hit() ? shadeHit(ShadingState.createRefractionBounceState(previous, r, i), Color.BLACK) : Color.BLACK;
        }
    ...
    /* ---------------------- O V E R A L L   "L I G H T"   D A R K N E S S ------------------------ */
    Color getIrradiance(ShadingState state, Color diffuseReflectance) {
        ...
        if (giEngine == null || state.getDiffuseDepth() >= maxDiffuseDepth){
            try {
                ...
                if(s instanceof GlassShader)
                    /* THIS IS THE PART MAKING GLASS LIGHTER/DARKER */
                    return Color.WHITE;
                else
                    return Color.BLACK;
            } catch (UnsupportedOperationException e) {
    ...
    /* ------------------------- G L A S S   E D G E S   D A R K N E S S -------------------------- */
    ConstantShader bs = null;
    Color traceReflection(ShadingState previous, Ray r, int i) {
        /* ---------- GLASS EDGES ---------- */
        /* MAYBE we need to add new "if()" statement which would deal with different colors */
        /* cos for examp[le trans-dark-blue needs DARK edges, wheres for example trans-yellow needs them BRIGHT */
        if (previous.getReflectionDepth() >= maxReflectionDepth || previous.getDiffuseDepth() > 0){
                return Color.BLACK;
        }
        IntersectionState istate = previous.getIntersectionState();
        ...
        // Background color
        ConstantShader cs = (ConstantShader) scene.sunflowAPI.lookupShader("background.shader");
        Color c = (cs == null) ? Color.BLACK : cs.getMaterialColor();
        return istate.hit() ? shadeHit(ShadingState.createReflectionBounceState(previous, r, i), c) : Color.BLACK;
    }
}