...
public final class ShadingState implements Iterable<LightSample> {
    ...
    public final Color shade() {
        return server.shadeHit(this, Color.BLACK);
    }
    ...
    public final HashMap getObjectCastingShadow(Ray r) {
        return server.getScene().objectCastingShadow(r, istate);
    }
    ...
    public final Color occlusion(int samples, float maxDist, Color bright, Color dark) {
        ...
        Color rezult = Color.black();
        for (int j = 0; j < samples; j++) {
            float xi = (float) getRandom(j, 0, samples);
            float xj = (float) getRandom(j, 1, samples);
            ...
            Ray raj = new Ray(p, w);
            raj.setMax(maxDist);
            rezult.add(Color.blend(bright, dark, traceShadow(raj)));
        }
        return rezult.mul(1.0f / samples);
    }
    ...
    public final Color specularPhong(Color spec, float power, int numRays) {
        ...
        if (numRays > 0) {
            ...
            for (int j = 0; j < numSamples; j++) {
                ...
                double r1 = getRandom(j, 0, numSamples);
                double r2 = getRandom(j, 1, numSamples);
                ...
                if (wn > 0)
                    lr.madd(wn * mul, traceGlossy(new Ray(p, w), j));
            }
        }
