...
public class TriangleMesh implements PrimitiveList {
    ...
    public boolean update(ParameterList pl, SunflowAPI api) {
        ...
        if (normals != null) {
            ...
            for (int i = 0; i<normals.data.length; i++){
                if (j<3) {
                    s += Float.toString(normals.data[i])+" ";
                    j++;
                }else{
                    q.w("NORMALS"+p+" "+s+" "+br);
                    j = 0;
        ...
        if (uvs != null) {
            ...
            //q.w(Integer.toString(uvs.data.length));
            /*
            String s = "";
            String br = "\r\n";
            int j = 1;
            int p = 1;
            int f = 0;
            for (int i = 0; i<uvs.data.length; i++){
                if (j<3) {
                    s += Float.toString(uvs.data[i])+" ";
                    j++;
                    if (p==1 && j==2)
                        f++;
                }else{
    ...
    public void prepareShadingState(ShadingState state) {
        //q.w(Integer.toString(this.uvs.data.length));
        state.init();
