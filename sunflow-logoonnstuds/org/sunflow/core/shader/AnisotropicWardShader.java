...
public class AnisotropicWardShader implements Shader {
    ...
    protected Color getDiffuse(ShadingState state) {
        return Shaders.getMixture(state, rhoD, "metallic");
    }
    ...
    public Color getRadiance(ShadingState state) {
        ...
        if (state.includeSpecular()) {
            ...
            for (LightSample sample : state) {
                ...
                lr.madd(cosNL * fr, Shaders.getMixture(state, sample.getSpecularRadiance(), "metallic")).clamp(0f, 1f);
            }
            ...
            if (numRays > 0) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    lr.madd(w / n, Shaders.getMixture(state, state.traceGlossy(r, i), "metallic")).clamp(0f, 1f);
                }
            }
            lr.mul(rhoS).clamp(0f, 1f);
        }
        // add diffuse contribution
        return lr.add(state.diffuse(getDiffuse(state))).clamp(0f, 1f);
    }
