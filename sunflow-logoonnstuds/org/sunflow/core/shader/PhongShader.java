...
public class PhongShader implements Shader {
    ...
    protected Color getDiffuse(ShadingState state) {
    	return Shaders.getMixture(state, diff);
    }
    */
    public Color getRadiance(ShadingState state) {
        ...
        // execute shader
        return state.diffuse(Shaders.getMixture(state, diff, "phong")).add(state.specularPhong(Shaders.getMixture(state, spec, "phong"), power, numRays));
    }

    public void scatterPhoton(ShadingState state, Color power) {
        // make sure we are on the right side of the material
        state.faceforward();
        Color d = Shaders.getMixture(state, diff, "phong");
