...
public class Texture {
    ...
    Texture(InputStream is, boolean isLinear) {
        this.filename = is.toString();
        this.isLinear = isLinear;
        
        UI.printInfo(Module.TEX, "Reading texture bitmap from: \"%s\" ...", filename);
        try {
			bitmap = new Bitmap(is, isLinear);
		} catch (IOException e) {
    ...
    public int getAlpha(float x, float y) {
        Bitmap bitmap = getBitmap();
        if (bitmap == null)
            return 0;

        return bitmap.getAlpha((int)(x*(bitmap.getWidth() - 1)),(int)(y* (bitmap.getHeight() - 1)));
    }
