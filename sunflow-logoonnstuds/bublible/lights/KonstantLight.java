package bublible.lights;

import bublible.Alerts;
...
public class KonstantLight extends Alerts implements LightSource, PrimitiveList, Shader {

    private boolean addLongShadows;
    ...
    public boolean update(ParameterList pl, SunflowAPI api) {
        ...
        shadow = pl.getFloat("shadow", shadow);
        addLongShadows = pl.getBoolean("addLongShadows", addLongShadows);
        autoColor = pl.getBoolean("autoColor", autoColor);
        // recompute model
        initSunSky();
        return true;
    }
    ...
    public void getSamples(ShadingState state) {
        ...
        // FARBA SVETLA
        //Color radiance = new Color("ffffff");
        ...
        //System.out.println("background color = " + SCParser.bgColor);
        if (Vector3.dot(sunDirWorld, state.getGeoNormal()) > 0 && Vector3.dot(sunDirWorld, state.getNormal()) > 0) {
            LightSample dest = new LightSample();
            dest.setShadowRay(new Ray(state.getPoint(), sunDirWorld));
            dest.getShadowRay().setMax(Float.MAX_VALUE);
        ...
        for (int i = 0; i < n; i++) {
            ...
            /*if (i == 0 && addLongShadows) {
                if (Vector3.dot(sunDirWorld, state.getGeoNormal()) > 0 && Vector3.dot(sunDirWorld, state.getNormal()) > 0) {
                    ...
                    dest.getDiffuseRadiance().mul(invP + power);
                    dest.getSpecularRadiance().mul(invP + power);
                    dest.traceShadow(state);
                    state.addSample(dest);
                }
            }*/
            ...
            if (Vector3.dot(dir, state.getGeoNormal()) > 0 && Vector3.dot(dir, state.getNormal()) > 0) {
                ...
                // DLZKA A TVAR TIENA
                //dest.setShadowRay(new Ray(state.getPoint(), new Vector3(dir.x - (shadow * 2), dir.y + shadow, dir.z + (shadow * 4))));
